...
void ScoreRenderer::draw(Img& canvas, const std::shared_ptr<model::GameState>& state) {
    ...
    int b_score_offset = (black_score_str.length() > 1) ? 28 : 38;
    canvas.put_text(black_score_str, b_score_offset, 430, 1.0, cv::Scalar(255, 255, 255), 3);

    canvas.draw_rect(10, 460, 80, 1, cv::Scalar(40, 40, 40), -1, 1.0);
    ...
    int w_score_offset = (white_score_str.length() > 1) ? 928 : 938;
    canvas.put_text(white_score_str, w_score_offset, 430, 1.0, cv::Scalar(255, 255, 255), 3);

    canvas.draw_rect(910, 460, 80, 1, cv::Scalar(40, 40, 40), -1, 1.0);
}
