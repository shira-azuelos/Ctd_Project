...
bool PieceRules::is_valid_geometry(const model::Board& board, model::PieceKind kind, model::PieceColor color, const model::Position& src, const model::Position& dest, bool is_capturing) {
    ...
    switch (kind) {
        ...
        case model::PieceKind::PAWN: {
            ...
            int start_row = (color == model::PieceColor::WHITE) ? board.get_height() - 2 : 1;
...
bool PieceRules::is_path_clear(const model::Board& board, const model::Position& src, const model::Position& dest) {
    ...
    int d_row = dest.row - src.row;
    int d_col = dest.col - src.col;

    int step_row = (d_row == 0) ? 0 : (d_row > 0 ? 1 : -1);
    int step_col = (d_col == 0) ? 0 : (d_col > 0 ? 1 : -1);

    model::Position curr(src.row + step_row, src.col + step_col);
    
    while (curr.row != dest.row || curr.col != dest.col) {
        if (board.get_piece_at(curr) != nullptr) return false;
        curr.row += step_row;
        curr.col += step_col;
    }
    return true;
